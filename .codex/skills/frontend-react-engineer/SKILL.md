...
description: Implement, refactor, debug, optimize, and verify production React frontends with TypeScript. Use for React components, hooks, routing, state, forms, MUI, SCSS/CSS, responsive UI, accessibility, performance, frontend architecture, tests, and build-tool changes in this repository.
---
...
4. Follow the existing stack and conventions. Do not introduce a dependency when the current platform can solve the task cleanly.

## Implement frontend work

- Respect the Deep Tree entity, layer, sharing, naming, controller, Context, and Manager rules.
...
- Use memoization only when referential stability is required or a measurable render cost justifies it. Do not scatter `useMemo`, `useCallback`, or `memo` defensively.
...
1. Run the relevant TypeScript check, lint, tests, and production build.
2. Start the application locally after functionality changes and inspect terminal output.
3. For UI changes, inspect the affected flow and browser console. Check desktop, tablet, and mobile when layout is involved.
4. Check the final diff for accidental scope changes and unresolved markers.
5. Do not report completion while new errors or warnings caused by the change remain unresolved. Clearly distinguish unrelated pre-existing issues.
